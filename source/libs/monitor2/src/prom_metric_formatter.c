...
int prom_metric_formatter_load_sample(prom_metric_formatter_t *self, prom_metric_sample_t *sample, int64_t ts) {
  PROM_ASSERT(self != NULL);
  if (self == NULL) return 1;
...
int prom_metric_formatter_load_metric(prom_metric_formatter_t *self, prom_metric_t *metric, int64_t ts) {
  PROM_ASSERT(self != NULL);
  if (self == NULL) return 1;
  ...
  for (prom_linked_list_node_t *current_node = metric->samples->keys->head; current_node != NULL;
    ...
    if (metric->type == PROM_HISTOGRAM) {
      ...
      for (prom_linked_list_node_t *current_hist_node = hist_sample->l_value_list->head; current_hist_node != NULL;
        ...
        r = prom_metric_formatter_load_sample(self, sample, ts);
        if (r) return r;
      }
    } else {
...
int prom_metric_formatter_load_metrics(prom_metric_formatter_t *self, prom_map_t *collectors, int64_t ts) {
  PROM_ASSERT(self != NULL);
  int r = 0;
  for (prom_linked_list_node_t *current_node = collectors->keys->head; current_node != NULL;
    ...
    for (prom_linked_list_node_t *current_node = metrics->keys->head; current_node != NULL;
      ...
      r = prom_metric_formatter_load_metric(self, metric, ts);
      if (r) return r;
    }
  }
  return r;
