...
prom_collector_registry_t *prom_collector_registry_new(const char *name) {
  ...
  self->out = prom_string_builder_new();
...
const char *prom_collector_registry_bridge(prom_collector_registry_t *self, int64_t ts) {
  prom_metric_formatter_clear(self->metric_formatter);
  prom_metric_formatter_load_metrics(self->metric_formatter, self->collectors, ts);
  char *out = prom_metric_formatter_dump(self->metric_formatter);

  int r = 0;
  r = prom_string_builder_add_str(self->out, out);
  if (r) return NULL;
  prom_free(out);

  return prom_string_builder_str(self->out);
}

int prom_collector_registry_clear_out(prom_collector_registry_t *self){
  return prom_string_builder_clear(self->out);
}
