...
static int32_t mndProcessCreateUserReq(SRpcMsg *pReq) {
  ...
  auditRecord(pReq, pMnode->clusterId, "createUser", createReq.user, "", createReq.sql);
...
static int32_t mndProcessAlterUserReq(SRpcMsg *pReq) {
  ...
  if(alterReq.alterType == TSDB_ALTER_USER_PASSWD){
    ...
    auditRecord(pReq, pMnode->clusterId, "alterUser", alterReq.user, "", alterReq.sql);
  }
    ...
    if (strcmp(alterReq.objname, "1.*") != 0){
      ...
      auditRecord(pReq, pMnode->clusterId, "GrantPrivileges", alterReq.user, name.dbname, alterReq.sql);
    }else{
      auditRecord(pReq, pMnode->clusterId, "GrantPrivileges", alterReq.user, "*", alterReq.sql);
    }
  }
  else if(alterReq.alterType == TSDB_ALTER_USER_ADD_SUBSCRIBE_TOPIC){
    auditRecord(pReq, pMnode->clusterId, "GrantPrivileges", alterReq.user, alterReq.objname, alterReq.sql);
  }
  else if(alterReq.alterType == TSDB_ALTER_USER_REMOVE_SUBSCRIBE_TOPIC){
    auditRecord(pReq, pMnode->clusterId, "RevokePrivileges", alterReq.user, alterReq.objname, alterReq.sql);
  }
  else{
    if (strcmp(alterReq.objname, "1.*") != 0){
      SName name = {0};
      tNameFromString(&name, alterReq.objname, T_NAME_ACCT | T_NAME_DB);
      auditRecord(pReq, pMnode->clusterId, "RevokePrivileges", alterReq.user, name.dbname, alterReq.sql);
    }else{
      auditRecord(pReq, pMnode->clusterId, "RevokePrivileges", alterReq.user, "*", alterReq.sql);
    }
  }
